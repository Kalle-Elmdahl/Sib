...
function createUrl(page, changefreq, priority = 0.5, lastMod = "2021-02-24") {
...
router.get('/sitemap.xml', async (req, res) => {
    ...
    const categoryLinks = [...categories, ...subCategories].map(category => category.link)
    ...
    articles.forEach(article => content.push(createUrl(`/${article.categoryLink}/article/${article.link}`, "monthly", 0.6, article.lastmod.split('T')[0])))
    let xml_content = [
